...
private:
    ...
    vector<int> avoidFlood(vector<int>& rains) {
        unordered_map<int, int> hashFloodingLake;
        const int numDays = rains.size();
        FOR (i, numDays) {
            const int rainingLake = rains[i];
            (void)itsRaining(hashFloodingLake, rainingLake);
        }
        
        for (auto itFl=begin(hashFloodingLake); itFl!=end(hashFloodingLake);) {
            if (itFl->second <= 1) {
                auto itTmp = itFl;
                ++itFl;
                hashFloodingLake.erase(itTmp);
            }
            else {
                ++itFl;
            }
        ...
        unordered_map<int, int> hashCurLake;
        vi viAns;
        FOR (i, numDays) {
            const int rainingLake = rains[i];
            if (rainingLake == 0) {
                bool isDried = false;
                FOR_INC(j, i+1, numDays) {
                    const int nextLake = rains[j];
                    if (nextLake == 0) continue;
                    auto itFl = hashFloodingLake.find(nextLake);
                    if (itFl != end(hashFloodingLake)) {
                        if (itFl->second <= 1) {
                            continue;
                        }
                        const int flLake = itFl->first;
                        auto it = hashCurLake.find(flLake);
                        if (it != end(hashCurLake)) {
                            const int fullLake = it->first;
                            hashCurLake.erase(it);
                            itFl->second--;
                            if (1 >= itFl->second) {
                                hashFloodingLake.erase(itFl);
                            }
                            isDried = true;
                            viAns.push_back(flLake);
                            break;
                        }
                    }
                }
                ...
                viAns.push_back(-1);
                if (itsRaining(hashCurLake, rainingLake) == false) {
                    return vi();
                    //return viAns;
                }
            }
    ...
    bool itsRaining(unordered_map<int, int>& hashLake, const int rainingLake) {
        ...
        if (end(hashLake) != it) {
            ...
            // first rain
            hashLake.emplace(make_pair(rainingLake, 1));
        }
        return true;
    }
#if 0   // 70yy
